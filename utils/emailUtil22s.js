// Исправляем импорт для Node.js v24
let nodemailer;
try {
  nodemailer = require('nodemailer');
} catch (error) {
  console.error('❌ Error importing nodemailer:', error);
  throw new Error('Failed to import nodemailer. Please check installation.');
}
...
if (typeof nodemailer.createTransporter !== 'function') {
  ...
  try {
    ...
    console.error('❌ Alternative import failed:', defaultError);
    throw new Error('Nodemailer is not properly imported. Please check installation.');
  }
}
